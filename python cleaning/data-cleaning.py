...
for _, row in df_sheet1.iterrows():
    ...
    if pd.notna(row["Topic"]):
        ...
        current_survey = row["Survey Name (Date)"].strip() if pd.notna(row["Survey Name (Date)"]) else ""
    ...
    if pd.notna(row["Question"]):
        ...
        # Create a unique identifier for each question using topic, survey, and question text
        current_question_id = f"{current_question}"
    ...
    if pd.notna(row["Response Option"]) and current_question_id is not None:
